...
function AdminLayout() {
  ...
  if (loading || !user || !isAdmin) {
    return <SiteShell><div className="px-6 py-32 text-center text-muted-foreground">Verifying access…</div></SiteShell>;
  }

  return (
    <SiteShell>
      <div className="px-6 pt-10 max-w-7xl mx-auto">
        <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-crimson mb-3">— Admin Console</p>
        <h1 className="text-3xl md:text-4xl font-bold tracking-tight">Platform control</h1>
        <nav className="mt-6 flex gap-1 border-b border-border overflow-x-auto">
          {[
            { to: "/admin", label: "Listings", exact: true },
